...
namespace Qrakhen.Sqript {
	internal class Interpretoken : Digester<Token> {
		...
		public Token[] readBody(bool includeBrackets = false, string until = "") {
			...
			if(includeBrackets)
				buffer.Add(peek());
			string ascend = digest().str(), descend = "";
			if(until != "")
				descend = until;
			else
				switch(ascend) {
					...
					case "<":
						...
						break;
					default:
						throw new ParseException("could not find closing element for opened '" + ascend + "'", peek());
				}
			do {
		...
		public Reference[] rrra<T>(Collection<T> context, int __l = 0, List<Reference> __buffer = null, Token nextToken = null) {
			...
			if((Keywords.isAlias(t.str(), Keyword.CURRENT_CONTEXT) && __l == 0)
				...
				if(nextToken != null) {
					...
					r = new Reference(
						new Value(context.value.Count, ValueType.Integer),
						null,
						this.stack[0] + ".len",
						Access.PUBLIC,
						true,
